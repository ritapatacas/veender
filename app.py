...
def init_mediapipe():
    ...
    return mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5), mp_face_mesh.FaceMesh(
...
def extract_face_features(image):
    ...
    if results.multi_face_landmarks:
        ...
        features = []
        for landmark in landmarks.landmark[:468]:
...
# --- NOVO BOTÃO DE TESTE DE STREAM ---
st.subheader("🔍 Quick Video Test (Stream Mode)")
test_url = st.text_input("Test YouTube link", placeholder="https://youtube.com/watch?v=...")
if st.button("▶️ Test Video Stream", use_container_width=True):
    if not test_url:
        st.error("⚠️ Please provide a YouTube link.")
    else:
        try:
            st.write("Obtaining direct stream URL with yt-dlp...")
            result = subprocess.run(["yt-dlp", "-g", test_url], capture_output=True, text=True, timeout=60)
            if result.returncode != 0:
                st.error(f"Failed to get stream URL:\n{result.stderr}")
            else:
                stream_url = result.stdout.strip()
                st.write(f"Stream URL obtained: {stream_url[:100]}...")

                cap = cv2.VideoCapture(stream_url)
                if not cap.isOpened():
                    st.error("Could not open stream. Possibly blocked (403) or unsupported format.")
                else:
                    fps = cap.get(cv2.CAP_PROP_FPS)
                    if fps == 0 or math.isnan(fps):
                        fps = 30
                    frame_interval = int(fps)
                    st.info(f"Detected FPS: {fps}")
                    ...
                    while True:
                        ...
                        if frame_count % frame_interval == 0:
                            ...
                            with preview_container:
                                st.image(frame_rgb, caption=f"Frame #{saved_frames}", use_container_width=True)
                            if saved_frames >= 5:
                                break  # Mostrar apenas 5 frames para teste
                        frame_count += 1

                    cap.release()
                    st.success(f"Test complete! Extracted {saved_frames} frames.")
        except subprocess.TimeoutExpired:
            st.error("Timeout while getting stream URL.")
        except Exception as e:
            st.error(f"Unexpected error: {str(e)}")

# --- RESTANTE CÓDIGO ORIGINAL AQUI (RUN VEENDER NORMAL) ---
